...
useEffect(() => {
  if (state.product?.images && state.product.images.length > 1) {
  ...
  const handleScroll = (event: any) => {
    ...
    scrollTrackingTimer.current = setTimeout(() => {
      if (state.product && currentScrollDepth > 50) {
...
const startImageAutoScroll = () => {
  ...
  autoScrollTimer.current = setInterval(() => {
    setCurrentImageIndex(prevIndex => {
      const nextIndex = (prevIndex + 1) % state.product!.images.length;
      return nextIndex;
    });
  }, 3500);
};
...
const ImageGallery: React.FC<ImageGalleryProps> = ({ images, onImagePress }) => {
  ...
  const scrollToImage = (index: number, animated: boolean = true) => {
  ...
  useEffect(() => {
    scrollToImage(currentImageIndex, true);
  }, [currentImageIndex]);

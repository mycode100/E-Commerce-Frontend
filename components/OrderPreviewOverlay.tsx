...
const OrderPreviewOverlay: React.FC<OrderPreviewOverlayProps> = ({
  ...
  const [internalPaymentMethod, setInternalPaymentMethod] = useState("COD");
  const [buttonScale] = useState(new Animated.Value(1));
  const [buttonProgress] = useState(new Animated.Value(0));

  const currentPaymentMethod = onPaymentMethodChange ? selectedPaymentMethod : internalPaymentMethod;
  ...
  const subtotal = bagSummary?.subtotal || bagItemsArray.reduce((sum, item) => sum + (item.quantity * item.priceWhenAdded), 0);
  const shipping = bagSummary?.shipping ?? (subtotal >= 499 ? 0 : 99);
  ...
  const handleClose = () => {
    if (isPlacingOrder || isLoading) return;
    onClose();
  };
    ...
    <View style={styles.summarySection}>
      ...
      <View style={styles.summaryContent}>
        <View style={styles.summaryRow}>
          <Text style={styles.summaryLabel}>Subtotal ({bagItemsArray.length} items)</Text>
      ...
      {paymentMethods.map((method) => (
        <TouchableOpacity
          ...
          style={[
            ...
            currentPaymentMethod === method.id && styles.selectedPaymentMethod,
            !method.enabled && styles.disabledPaymentMethod,
          ]}
          onPress={() => method.enabled && handlePaymentMethodChange(method.id)}
        ...
        >
          ...
          <View
            style={[
              styles.radioButton,
              currentPaymentMethod === method.id && styles.radioButtonSelected,
            ]}
          >
  ...
  const renderPlacingOrder = () => (
    <View style={styles.placingOrderContainer}>
      ...
      <Text style={styles.placingOrderText}>
        Please wait while we process your order
      </Text>
      <View style={styles.processingDetails}>
        <Text style={styles.processingStep}>📦 Validating items</Text>
        <Text style={styles.processingStep}>💳 Processing payment</Text>
        <Text style={styles.processingStep}>🚚 Arranging delivery</Text>
      </View>
    </View>
  );
      ...
      <View style={styles.container}>
          ...
          >
            <View style={styles.itemsSection}>
              <View style={styles.itemsHeader}>
                ...
                <Text style={styles.sectionTitle}>Items ({bagItemsArray.length})</Text>
              </View>
              {bagItemsArray.length === 0 ? (
                <View style={styles.emptyBagContainer}>
                  ...
                  <Text style={styles.emptyBagText}>
                    Add items to your bag to place an order
                  </Text>
                </View>
              ) : (
                <FlatList
        ...
        {orderStep === "preview" && (
          <View style={styles.footer}>
            <Animated.View
              style={[
                styles.placeOrderButtonContainer,
            ...
            >
              ...
              >
                <Animated.View
                  style={[styles.buttonProgressBar, { width: progressWidth }]}
                />
